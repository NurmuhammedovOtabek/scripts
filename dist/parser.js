...
exports.parseRegistrHtml = parseRegistrHtml;
...
const norm = (s) => clean(s)
    ...
    .replace(/[\u201C\u201D\u201E]/g, '"')
    .replace(/\s+/g, ' ');
function getByLabel($, root, labels) {
    ...
    for (const want of labels) {
        ...
        for (const td of tds) {
            ...
            if (txt.includes(wantN)) {
                ...
                if (valueTd.length)
                    return clean(valueTd.text());
            }
        }
    }
    return null;
}
function parseRegistrHtml(html) {
    const $ = (0, cheerio_1.load)(html);
    const root = $('#demo2');
    if (!root.length)
        throw new Error('demo2 topilmadi');
    const founders = [];
